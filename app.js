...
const BudgetChart = mongoose.model("BudgetChart", budgetSchema);
...
app.get("/userHome", (req, res) => {
    res.render("userHome.ejs", {resStr: result, userPrompt: prompt});
});

var chatBotFile;
app.post("/chatBotPDF", upload1.single('tender'), (req, res) => {
    console.log(req.file.filename);
    chatBotFile = req.file.filename;
    res.redirect("userHome");
})

app.post("/prompt", async (req,res) => {
    //console.log(req.body);
    const pdfPath = "C:/Users/hp/OneDrive/Desktop/Web Development/HackForge---ByteKarma---1B/public/tenders/" + chatBotFile;
    prompt = req.body.prompt;
    result = await axios.post(api_uri+ "/chatbot", {
            Question: prompt,
            ChatBotPDF: pdfPath
        });
...
app.get("/addNewProject", function(req, res) {
    res.render("govAddProj.ejs", {states: states, sectorList:sectors, pdfData:pdfInfo});
});
...
app.get("/:srNo", async (req, res) => {
    if(req.params.srNo !== "favicon.ico"){
        ...
        var matchedArr4 = await BudgetChart.find({srNoChart: serialNo});
        console.log(matchedArr3);
        if(matchedArr3.length === 0 && matchedArr4.length === 0){
            res.render("projDisplay.ejs", {info: matchedArr1, info1: matchedArr2, info2: [], info3: [], info4: [], info5: [], userLoggedIn: userLog, govLoggedIn: govLog, queryAns:queryResponse, que:projQue});    
        } 
        else if(matchedArr3.length === 0 && matchedArr4.length !== 0){
            res.render("projDisplay.ejs", {info: matchedArr1, info1: matchedArr2, info2: [], info3: [], info4: matchedArr4[0].xaxisVal, info5: matchedArr4[0].yaxisVal, userLoggedIn: userLog, govLoggedIn: govLog, queryAns:queryResponse, que:projQue});    
        }
        else if(matchedArr3.length !== 0 && matchedArr4.length === 0){
            res.render("projDisplay.ejs", {info: matchedArr1, info1: matchedArr2, info2: matchedArr3[0].xaxisVal, info3: matchedArr3[0].yaxisVal, info4: [], info5: [], userLoggedIn: userLog, govLoggedIn: govLog, queryAns:queryResponse, que:projQue});
        }
        else{
            res.render("projDisplay.ejs", {info: matchedArr1, info1: matchedArr2, info2: matchedArr3[0].xaxisVal, info3: matchedArr3[0].yaxisVal, info4: matchedArr4[0].xaxisVal, info5: matchedArr4[0].yaxisVal, userLoggedIn: userLog, govLoggedIn: govLog, queryAns:queryResponse, que:projQue});
        }
    }
});

app.post("/projQuery", async(req,res) => {
    var matchedProj = await projInfo.find({srNo: serialNo});
    var projFile = matchedProj[0].fileName;
    console.log(projFile);
    const projPdfPath = "C:/Users/hp/OneDrive/Desktop/Web Development/HackForge---ByteKarma---1B/public/tenders/" + projFile;
    projQue = req.body.prompt;
    queryResponse = await axios.post(api_uri+ "/chatbot", {
            Question: projQue,
            ChatBotPDF: projPdfPath
        });
    console.log(queryResponse.data.answer);
    queryResponse = queryResponse.data.answer;
...
app.post("/trackbudget", async (req, res) => {
    budMonths.push(req.body.month);
    budUsed.push(req.body.percent);
    const chartFound = await BudgetChart.find({srNoChart: serialNo});
    if(chartFound.length === 0){
        ...
        const storeX = await BudgetChart.updateOne({srNoChart: serialNo}, {$push: {xaxisVal: req.body.month}});
        const storeY = await BudgetChart.updateOne({srNoChart: serialNo}, {$push: {yaxisVal: req.body.percent}});
        console.log(storeX);
        console.log(storeY); 
    }
    budMonths = [];
...
var pdfInfo = {}; 
var savedFileName;
app.post("/uploadPDF", upload1.single('tender'), async(req, res) => {
    savedFileName = req.file.filename;
    const result = await axios.post(api_uri+ "/summarize",{
        pdfName: "C:/Users/hp/OneDrive/Desktop/Web Development/HackForge---ByteKarma---1B/public/tenders/" + savedFileName
    });
...
app.post('/delete-issue', async(req,res) => {
    ...
    await reportedIssue.deleteOne({_id:issueId}).then(()=> {
